...
class FileList():
    ...
    def findfile(self, gps, ifo):
        ...
        if len(filenamelist) == 0:
            print("WARNING!  No file found for GPS {0} and IFO {1}".format(
                gps, ifo))
            return None
        else:
...
def getsegs(start, stop, ifo, flag='DATA', filelist=None):
    ...
    for time in gpsList:
        ...
        if filename is None:
            print(
                "WARNING! No file found with GPS start time {0}".format(time))
            print("Segment list may contain errors due to missing files.")
            continue
        else:
            try:
                strain, meta, dq = loaddata(
                    filename, ifo, tvec=False, readstrain=False)
            except:
        ...
        i_start = meta['start']
        i_seglist = [(indx.start + i_start, indx.stop + i_start)
